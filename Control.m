...
@interface	CommandInfo : ClientInfo
{
  NSArray	*servers;
}
...
@interface	ConsoleInfo : ClientInfo
{
  NSString	*cserv;
  NSString	*chost;
  NSString	*pass;
...
@interface	Control : EcProcess <Control>
{
  NSFileManager		*mgr;
  ...
  if (inTimeout == NO)
    {
      ...
      while (count-- > 0)
	{
	  ClientInfo	*r = [commands objectAtIndex: count];
      ...
      while (count-- > 0)
	{
	  ClientInfo	*r = [consoles objectAtIndex: count];
	  NSDate	*d = [r lastUnanswered];
...
{
  NSString	*m;
  ClientInfo	*o = [self findIn: commands byObject: obj];
...
inner_main()
{
  ...
  server = [[Control alloc] initWithDefaults: defs];
  if (server == nil)
    {
