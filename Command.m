...
@interface	Command : EcProcess <Command>
{
  NSString		*host;
  id<Control>		control;
...
- (void) newConfig: (NSMutableDictionary*)newConfig
{
  ...
  if (nil == config || [config isEqual: newConfig] == NO)
      ...
      else
	{
      ...
      while (i-- > 0)
	{ 
	  ClientInfo	*c = [a objectAtIndex: i];
	    ...
	    from: (NSString*)f
{
  ...
  if (cmd == nil || [cmd count] == 0)
	  ...
	  if ([wd length] == 0 || comp(wd, @"clients") >= 0)
	      ...
	      else
		{
		  ...
		  for (i = 0; i < [clients count]; i++)
		    {
		      ClientInfo*	c = [clients objectAtIndex: i];
    ...
    {
      ClientInfo	*client = [self findIn: clients byName: t];
...
- (id) connectionBecameInvalid: (NSNotification*)notification
{
  ...
  if ([conn isKindOfClass: [NSConnection class]])
      ...
      if (control && [(NSDistantObject*)control connectionForProxy] == conn)
	{
      ...
      while (i-- > 0)
	{
	  ClientInfo*	o = [clients objectAtIndex: i];
      ...
      byAbbreviation: (NSString*)s
{
  ...
  if (isdigit(*[s cString]))
	...
	{
	  o = (ClientInfo*)[a objectAtIndex: i];
	...
	byAbbreviation: (NSString*)s
{
  ...
  if (isdigit(*[s cString]))
    {
      ...
      if (i >= 0 && i < (int)[a count])
	{
	  return (ClientInfo*)[a objectAtIndex: i];
	}
    }
  ...
  if (best_pos >= 0)
    {
      return (ClientInfo*)[a objectAtIndex: best_pos];
    }
  return nil;
}
  ...
  if (launchInfo != nil)
    {
      ...
      while ((key = [enumerator nextObject]) != nil)
	{
	  ClientInfo	*r = [self findIn: clients byName: key];
...
- (void) logMessage: (NSString*)msg
		...
		for: (id<CmdClient>)o
{
  ClientInfo*		r = [self findIn: clients byObject: o];
  NSString		*c;
...
- (void) requestConfigFor: (id<CmdConfig>)c
{
  ClientInfo	*info = [self findIn: clients byObject: (id<CmdClient>)c];
		 ...
		 transient: (BOOL)t
{
   ...
   */
  obj = [[ClientInfo alloc] initFor: c name: n with: self];
...
- (void) timedOut: (NSTimer*)t
{
  ...
  if (inTimeout == NO)
      ...
      if (control == nil)
	{
      ...
      while (count-- > 0)
	{
	  ClientInfo	*r = [clients objectAtIndex: count];
...
- (void) unregisterByObject: (id)obj
{
  ClientInfo	*o = [self findIn: clients byObject: obj];
...
- (void) unregisterByName: (NSString*)n
{
  ClientInfo	*o = [self findIn: clients byName: n];
...
#define	CORESIZE	250000000
  ...
  server = [[Command alloc] initWithDefaults: defs];
  if (server == nil)
    {
